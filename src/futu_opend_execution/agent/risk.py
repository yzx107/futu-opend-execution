...
class RealOrderGuard:
    ...
    def validate(
        ...
        *,
        ...
        self._check_real_mode(execution_mode, confirm_text, enable_auto_cost_reducer, approved)
    ...
    def _check_inventory(self, intent: RealOrderIntent, inventory: InventoryState | None) -> None:
        ...
        if intent.role is OrderRole.TRADING_SELL and intent.quantity > inventory.trading_available_to_sell:
            raise ExecutionValidationError("sell quantity exceeds trading inventory")
        if intent.role is OrderRole.TRADING_REBUY and intent.quantity > inventory.trading_available_to_rebuy:
