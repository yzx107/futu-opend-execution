...
from futu_opend_execution.services import cost_reducer
...
class CostReducerTests(unittest.TestCase):
    ...
    def test_cost_reducer_engine_does_not_import_trading_position_engine(self) -> None:
        self.assertNotIn("TradingPositionEngine", vars(cost_reducer))
        self.assertNotIn("TradingPositionRules", vars(cost_reducer))

    def test_cost_reducer_engine_does_not_call_trading_position_engine(self) -> None:
        inventory = split_inventory_targets(total_quantity=1000, lot_size=100)
        inventory.seed_opening_inventory(anchor_price="10")
        with patch(
            "futu_opend_execution.services.trading_position.TradingPositionEngine.evaluate",
            side_effect=AssertionError("must not be called"),
        ):
            decision = CostReducerEngine(CostReducerRules(min_ticks_to_activate=1)).evaluate(
                inventory=inventory,
                market=self._market(),
                state=CostReducerState(),
            )
        self.assertEqual(decision.action, CostReducerAction.SELL_TRADING)
    ...
    def test_sell_decision_creates_sell_trading_executable_intent(self) -> None:
        ...
        intent = build_executable_intent(
            decision=CostReducerDecision(CostReducerAction.SELL_TRADING, 100, "sell"),
    ...
    def test_rebuy_decision_creates_buy_rebuy_executable_intent_with_edge(self) -> None:
        inventory = split_inventory_targets(total_quantity=1000, lot_size=100)
        inventory.seed_opening_inventory(anchor_price="10")
        ...
        intent = build_executable_intent(
            decision=CostReducerDecision(CostReducerAction.REBUY_TRADING, 100, "rebuy"),
            ...
            policy=CostReducerExecutionPolicy(
                ...
                min_expected_edge_bps=Decimal("300"),
            ),
            best_bid=Decimal("10.09"),
            best_ask=Decimal("10.10"),
            last_sell_price=Decimal("10.50"),
        )

        self.assertEqual(intent.side, GreyOrderSide.BUY)
        self.assertEqual(intent.role, GreyOrderRole.TRADING_REBUY)
        self.assertEqual(intent.limit_price, Decimal("10.11"))
        self.assertGreater(intent.expected_edge_bps, Decimal("300"))
        self.assertEqual(intent.status, CostReducerExecutableStatus.PENDING_APPROVAL)

    def test_rebuy_executable_blocks_without_last_sell_price(self) -> None:
        inventory = split_inventory_targets(total_quantity=1000, lot_size=100)
        inventory.seed_opening_inventory(anchor_price="10")
        inventory.record_trading_sell(quantity=100, price="10.5")
        ...
        intent = build_executable_intent(
            ...
            inventory=inventory,
            rules=CostReducerRules(min_ticks_to_activate=1),
            policy=CostReducerExecutionPolicy(dry_run_only=False),
            best_bid=Decimal("10.09"),
            best_ask=Decimal("10.10"),
        )

        self.assertEqual(intent.status, CostReducerExecutableStatus.BLOCKED)
        self.assertIn("last_sell_price", intent.reason)

    def test_rebuy_executable_blocks_when_expected_edge_too_low(self) -> None:
        inventory = split_inventory_targets(total_quantity=1000, lot_size=100)
        inventory.seed_opening_inventory(anchor_price="10")
        inventory.record_trading_sell(quantity=100, price="10.5")
        ...
        intent = build_executable_intent(
            ...
            policy=CostReducerExecutionPolicy(
                dry_run_only=False,
                require_positive_expected_edge=True,
                min_expected_edge_bps=Decimal("1000"),
            ),
            best_bid=Decimal("10.09"),
            best_ask=Decimal("10.10"),
            last_sell_price=Decimal("10.50"),
        ...
        self.assertEqual(intent.status, CostReducerExecutableStatus.BLOCKED)
        self.assertIn("expected edge", intent.reason)
