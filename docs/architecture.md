...
Futures support is kept as a parallel foundation, not a mutation of the stock inventory model. `ContractSpec` defines multiplier, tick size, min order size, margin rate, commission, sessions, expiry, and rollover group. `FuturesPaperLedger` applies BUY/SELL open/close fills with FIFO realized PnL, commission, open margin, and mark-to-market. `FutuOpenDFuturesClient` is read-only: it can call OpenD `get_future_info` and optionally open/close `OpenFutureTradeContext` as a capability probe. Real futures order placement is intentionally absent until contract metadata, session handling, broker behavior, and risk limits are validated separately.
